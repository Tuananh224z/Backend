...
const processCassoWebhook = async (headers, body) => {
  ...
  if (!secureToken || secureToken !== expectedToken) {
    console.warn('Cảnh báo bảo mật: Nhận Webhook Casso nhưng secure-token không đúng!');
  ...
  console.log('=== NHẬN GIAO DỊCH CHUYỂN KHOẢN MỚI TỪ CASSO ===');
  
  // Duyệt qua từng giao dịch ngân hàng do Casso gửi sang
  for (const transaction of data) {
    const { description, amount, tid, when } = transaction;
    console.log(`- Giao dịch ID: ${tid}`);
    console.log(`- Số tiền: ${amount.toLocaleString('vi-VN')}đ`);
    console.log(`- Nội dung chuyển khoản: "${description}"`);
    console.log(`- Thời gian: ${when}`);
    ...
    if (match) {
      ...
      console.log(`=> Phát hiện nội dung chuyển khoản khớp với mã đơn hàng: ${orderCode}`);
      ...
      if (order) {
        ...
        if (order.paymentStatus !== 'Paid') {
          ...
          if (amount >= order.totalAmount) {
            ...
            await order.save();
            console.log(`[THÀNH CÔNG] Đơn hàng ${orderCode} đã được tự động duyệt thanh toán qua Casso!`);
          } else {
            console.warn(`[THẤT BẠI] Số tiền chuyển khoản (${amount}đ) nhỏ hơn tổng giá trị đơn hàng (${order.totalAmount}đ)!`);
          }
        } else {
          console.log(`=> Đơn hàng ${orderCode} đã có trạng thái thanh toán là 'Paid' từ trước.`);
        }
      } else {
        console.warn(`=> Không tìm thấy đơn hàng trong hệ thống khớp với mã: ${orderCode}`);
      }
    } else {
      console.log('=> Nội dung chuyển khoản không chứa mã đơn hàng ORD-xxxxxxxx-xxxx hợp lệ.');
    }
  }
